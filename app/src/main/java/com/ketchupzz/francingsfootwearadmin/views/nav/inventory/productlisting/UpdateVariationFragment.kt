...
class UpdateVariationFragment : Fragment() {
    ...
    private lateinit var binding : FragmentUpdateVariationBinding
    private lateinit var loadingDialog: LoadingDialog
    private val variationViewModel by activityViewModels<VariationViewModel>()
    private val args by navArgs<UpdateVariationFragmentArgs>()
    ...
    override fun onCreateView(
        ...
        binding = FragmentUpdateVariationBinding.inflate(inflater,container,false)
        loadingDialog = LoadingDialog(binding.root.context)
        Glide.with(binding.root.context)
            .load(args.variation.image)
            .error(R.drawable.product)
            .into(binding.buttonAddImage)
        binding.inputVariationName.setText(args.variation.name)
        sizes.addAll(args.variation.sizes)
        sizes.map {
            addTextViewToRow(it)
        }
        return binding.root
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.buttonSaveVariation.setOnClickListener {
            ...
            if (selectedImage == null) {
                val variation = Variation(args.variation.id, image = args.variation.image,name = variationName,sizes)
                saveVariation(args.productID,variation)
            } else {
                val variation = Variation(args.variation.id, image = selectedImage.toString(),name = variationName,sizes)
